...
public class KDLNode implements KDLObject {
    ...
    public KDLNode(
        ...
        Map<String, KDLValue<?>> props,
        List<KDLValue<?>> args,
        Optional<KDLDocument> child
    ) {
     ...
     */
    @Override
    public void writeKDL(Writer writer, PrintConfig printConfig)
        throws IOException {
        writeKDLPretty(writer, 0, printConfig);
    }

    void writeKDLPretty(Writer writer, int depth, PrintConfig printConfig)
        throws IOException {
        if (type.isPresent()) {
            writer.write('(');
            writeStringQuotedAppropriately(
                writer,
                type.get(),
                true,
                printConfig
        ...
        for (var value : this.args) {
            if (
                !(value instanceof KDLNull) || printConfig.shouldPrintNullArgs()
            ) {
                writer.write(' ');
        ...
        for (var entry : props.entrySet()) {
            if (
                !(entry.getValue() instanceof KDLNull) ||
                printConfig.shouldPrintNullProps()
            ) {
                writer.write(' ');
                writeStringQuotedAppropriately(
                    writer,
        ...
        if (child.isPresent()) {
            if (
                !child.get().getNodes().isEmpty() ||
                printConfig.shouldPrintEmptyChildren()
            ) {
                writer.write(" {");
    ...
    public boolean equals(Object o) {
        ...
        return (
            Objects.equals(identifier, kdlNode.identifier) &&
            Objects.equals(type, kdlNode.type) &&
            Objects.equals(props, kdlNode.props) &&
            Objects.equals(args, kdlNode.args) &&
            Objects.equals(child, kdlNode.child)
        );
    }
    ...
    public static class Builder {
        ...
        public Builder removePropIf(Predicate<String> keyPredicate) {
            ...
            keysToRemove.stream().forEach(props::remove);
            return this;
        }
