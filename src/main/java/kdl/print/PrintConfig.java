...
import static kdl.parse.CharClasses.*;
...
public class PrintConfig {
    ...
    public static final PrintConfig PRETTY_DEFAULT = PrintConfig.builder()
        .build();
    ...
    private PrintConfig(
        Map<Integer, Boolean> escapes,
        boolean escapeNonPrintableAscii,
        boolean escapeLinespace,
        boolean escapeNonAscii,
        boolean escapeCommon,
        boolean requireSemicolons,
        boolean respectRadix,
        String newline,
        int indent,
        char indentChar,
        char exponentChar,
        boolean printEmptyChildren,
        boolean printNullArgs,
    ...
    public boolean requiresEscape(int c) {
        if (shouldForceEscape(c)) {
            ...
            escapeNonPrintableAscii && !isNonAscii(c) && !isPrintableAscii(c)
        ) {
            return true;
    ...
    public static class Builder {
        ...
        public Builder setEscapeNonPrintableAscii(
            boolean escapeNonPrintableAscii
        ) {
            this.escapeNonPrintableAscii = escapeNonPrintableAscii;
            return this;
        ...
        public PrintConfig build() {
            if (exponentChar != 'e' && exponentChar != 'E') {
                throw new IllegalArgumentException(
                    "Exponent character must be either 'e' or 'E'"
                );
            }
            ...
            for (int i = 0; i < newline.length(); i++) {
                if (!isUnicodeLinespace(newline.charAt(i))) {
                    throw new IllegalArgumentException(
                        "All characters in specified 'newline' must be unicode vertical space"
                    );
                }
            }
            ...
            if (!isUnicodeWhitespace(indentChar)) {
                throw new IllegalArgumentException(
                    "Indent character must be unicode whitespace"
                );
            }
